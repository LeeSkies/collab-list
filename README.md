...
Account email changes use Supabase Auth's confirmation flow. The local SMTP monitor is not wired into the Playwright harness, so email-change confirmation is covered by the API, component, and pgTAP tests rather than an E2E test; verify the confirmation messages manually through the local SMTP UI when exercising that flow locally.
